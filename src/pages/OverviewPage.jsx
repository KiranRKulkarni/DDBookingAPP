...
function Metric({ label, value, onClick }) {
  return <article className={onClick ? 'metric-link' : ''} onClick={onClick} onKeyDown={(event) => event.key === 'Enter' && onClick?.()} role={onClick ? 'button' : undefined} tabIndex={onClick ? 0 : undefined}><span>{label}</span><strong>{value}</strong>{onClick && <small>View records →</small>}</article>
}
...
export default function OverviewPage({ totals, bookings, month, setMonth, checkIn, checkingIn, checkOut, checkingOut, onPaymentFollowUps, onCashCollection, onOpenProperty }) {
  const propertyBreakdown = PROPERTIES.map((property) => {
    const propertyBookings = bookings.filter((booking) => booking.property === property)
